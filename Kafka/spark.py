...
def print_only_at_the_end(rdd):
    ...
    if empty_rdds_count >= THRESHOLD and not printed:
        for segment, count in rdd.map(lambda x: (x[0], len(x[1]))).sortBy(lambda x: x[1], ascending=False).collect():
            print("{}\t{}".format(segment, count))
            producer.send(segment, str(count))
